...
def mail_package_review_request_to_admins(context, data_dict, _type='new'):
    ...
    for admin_id in admin_ids:
        ...
        if user.email:
            ...
            body = _compose_email_body(data_dict, user, _type)
...
def _compose_email_body(data_dict, user, _type):
    ...
    email_body = f'''
        ...
        A {_type} dataset has been requested for review by {publisher_name}:

            {package_title}

            {package_description}

        To approve or reject the request, please visit the following page (logged in as a admin):
